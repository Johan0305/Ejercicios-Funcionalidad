...
let ferrari = () => cars.filter(({ brand }) => brand === "Ferrari");

console.log(ferrari());

let nineties = () => cars.filter(({ model }) => model >= 1980 && model <= 1990);

console.log(nineties());

let list = () =>
  cars.map(
    ({ brand, price, model }) => `This ${brand} from ${model} cost $${price}`
  );
...
let bmwSum = () =>
  cars
    .filter(({ brand }) => brand === "BMW")
    .map(({ price }) => price)
    .reduce((price, priceCount) => price + priceCount);
